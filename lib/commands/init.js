  ...
  cli.command('init')
    .handler(function (data, flags, done) {
      ...
      Generator.prototype.connect = function () {
        ...
        // Make docker volume mapped folders
        mkdirp.sync(this.destinationPath(path.join('connect', 'email')))
        mkdirp.sync(this.destinationPath(path.join('connect', 'logs')))
        mkdirp.sync(this.destinationPath(path.join('connect', 'protocols')))
        mkdirp.sync(this.destinationPath(path.join('connect', 'providers')))
        mkdirp.sync(this.destinationPath(path.join('connect', 'public')))
        mkdirp.sync(this.destinationPath(path.join('connect', 'views')))
      ...
      Generator.prototype.nginx = function () {
        if (this.nginx) {
          ...
          // Make docker volume mapped folders
          mkdirp.sync(this.destinationPath(path.join('nginx', 'logs')))
      ...
      Generator.prototype.redis = function () {
        if (this.redis) {
          ...
          // Make docker volume mapped folders
          mkdirp.sync(this.destinationPath(path.join('redis', 'data')))
          mkdirp.sync(this.destinationPath(path.join('redis', 'logs')))
      ...
      Generator.prototype.selfSignedCert = function () {
        ...
        if (this.ssl) {
          ...
          cert.on('exit', function () {
            ...
            self.log('   ' + chalk.green('create ') + 'nginx/certs/nginx.crt')
            callback()
          })
        }
      }
