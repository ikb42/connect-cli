...
function registerUser (cli, options, done) {
  ...
  userCmd
    .handler(function (data, flags, done) {
      ...
      cli.log('  nvl user:roles [<id>] [--issuer | -i <issuer id>]')

      cli.log('  nvl user:assign [<user id> <role name>] [--issuer | -i <issuer id>]')

      cli.log('  nvl user:revoke [<user id> <role name>] [--issuer | -i <issuer id>]')

      cli.log('  nvl user:token [--issuer | -i <issuer id>]')
                    ...
                    cli.prompt([
                      {
                        ...
                        }).filter(function (choice) { return choice !== null })
                      }
                    ], function (answers) {
  ...
  userCmd
    ...
    .handler(function (data, flags, done) {
      cli.issuers.prompt(flags['issuer'] || flags.i, function (err, issuer) {
        ...
        if (anvil.tokens === undefined || anvil.tokens.access_claims.exp * 1000 < Date.now()) {
          cli.log.error('Please login to the issuer.')
        } else {
          cli.log.json(anvil.tokens)
        }
