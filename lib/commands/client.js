...
function registerClient (cli, options, done) {
  ...
  clientCmd
    .handler(function (data, flags, done) {
      ...
      cli.log('  nvl client:roles [<id>] [--issuer | -i <issuer id>]')

      cli.log('  nvl client:assign [<client id> <role name>] [--issuer | -i <issuer id>]')

      cli.log('  nvl client:revoke [<client id> <role name>] [--issuer | -i <issuer id>]')
    ...
    .handler(function (data, flags, done) {
      cli.issuers.prompt(flags['issuer'] || flags.i, function (err, issuer) {
        ...
        anvil.discover()
          ...
          // retrieve the roles for this client
          .then(function (clientId) {
            return anvil.clients.roles.list(clientId, { token: token })
          })
    ...
    .handler(function (data, flags, done) {
      cli.issuers.prompt(flags['issuer'] || flags.i, function (err, issuer) {
        ...
        anvil.discover()
          ...
          // get role and client choices
          .then(function (configuration) {
            return new Promise(function (resolve, reject) {
              // command arguments for client and role
              if (data[0] && data[1]) {
                resolve({
                  clientId: data[0],
                  roleName: data[1]
              ...
              // prompt for client and role
              } else {
                Promise.all([
                  anvil.clients.list({ token: token }),
          ...
          .then(function (data) {
            ...
            return anvil.clients.roles.add(client, role, { token: token })
          })
      ...
      cli.issuers.prompt(flags['issuer'] || flags.i, function (err, issuer) {
        ...
        anvil.discover()
          ...
          .then(function (data) {
            ...
            return anvil.clients.roles.delete(client, role, { token: token })
          })
